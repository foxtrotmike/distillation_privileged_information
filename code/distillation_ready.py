...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 19 17:04:24 2018
...
#import theano.tensor as T as K
import numpy as np
#import theano as K
...
def MLP(d,q):
    model = Sequential()
    model.add(Dense(2*q, input_dim=d))
...
def distillation(d,q,t,l):
    ...
    w3 = Dense(q)(inputs)
    relux = Activation('tanh', name = 'relux')(w3)
    w4 = Dense(q)(relux)
...
#    graph.add_node(Activation('softmax'), name='soft_softmax', inpux_trt='w3')
...
#    graph.add_output(name='soft', input='soft_softmax')
    ...
    loss_hard = weighted_loss(mse,1.-l)
    loss_soft = weighted_loss(mse,t*t*l)
...
def do_exp(x_tr,xs_tr,y_tr,x_te,xs_te,y_te):
    ...
    mlp_priv.fit(xs_tr, y_tr, epochs=1000, verbose=0)
    ...
    mlp_reg.fit(x_tr, y_tr, epochs=1000, verbose=0)
    res_reg = np.mean(mlp_reg.predict_classes(x_te,verbose=0)==np.argmax(y_te,1))
#    print mlp_reg.summary()
#    print mlp_priv.summary()
    # distilled
    mlp_dist = distillation(x_tr.shape[1],2,t,l)
    for lx in mlp_priv.layers:
        lx.trainable = False
    #mlp_priv.layers[0].trainable = False
    soften = K.function([mlp_priv.layers[0].input,], [mlp_priv.layers[-2].output])
    
    p_tr   = softmax(soften([xs_tr.astype(np.float32),])[0],t)
    #import pdb; pdb.set_trace()
#    print mlp_priv.summary()
#    print mlp_dist.summary()
#    #1/0
    mlp_dist.fit(x_tr,{'hard':y_tr, 'soft':p_tr}, epochs=1000, verbose=0)
    res_dis = np.mean(np.argmax(mlp_dist.predict(x_te)[0],1)==np.argmax(y_te,1))
